...
function loadHTMLMessage(messageHtml,instanceId,messageId){const parser=new DOMParser();const doc=parser.parseFromString(messageHtml,'text/html');loadScriptWithCallback('bridge.min.js',function(){messageLoaded(instanceId,messageId);keepReportingSizeChanges()});document.head.innerHTML+=doc.head.innerHTML;document.body.innerHTML=doc.body.innerHTML;executeInlineScripts();}
function keepReportingSizeChanges(){clearInterval(sizeChangeReportingIntervalId);sizeChangeReportingIntervalId=setInterval(reportMessageSize,1000);}
function executeInlineScripts(){var scripts=document.body.getElementsByTagName('script');for(var i=0;i<scripts.length;i++){if(scripts[i].src){var tag=document.createElement('script');tag.src=scripts[i].src;document.head.appendChild(tag);}else{window.$eval(scripts[i].innerText);}}}
